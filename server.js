...
app.get("/api/explore/area/slot",async(req,res)=>{
  ...
  const availability = await prisma.parkingSlot.findMany({where:{id:area_det.id, isAvailable:true}, select:{slotNumber:true}});
  const slotList = availability.map((u)=>u.slotNumber);
  const available = slotList.length;
  res.status(200).json({reply : `There are ${area_det.totalSlots} in ${area} and ${available} slots are available`});
})
